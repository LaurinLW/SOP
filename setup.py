...
def setup_f(rpc, experimente):
    ...
    client.images.build(path='.', dockerfile='Dockerfile_experiment', tag='sop-experiment')

def config_f(rpc, experimente):
    cnf = read_settings()
    experiment_folder = [l for l in cnf if l.startswith('SHARED_EXPERIMENT')]
    if len(experiment_folder) > 0:
        cnf.remove(experiment_folder[0])

    rpc_path = [l for l in cnf if l.startswith('RPC_PATH')]
    if len(rpc_path) > 0:
...
def start_f():
    ...
    experiment_folder = [l for l in cnf if l.startswith('SHARED_EXPERIMENT')][0].split(' = ')[1][1:-2]
